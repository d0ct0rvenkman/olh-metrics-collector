...
// fetchGpuTemp gets the GPU temperature from the /api/gpuTemp/clean endpoint.
func fetchGpuTemp(ctx context.Context, cfg *ClientConfig) (float64, error) {
	apiURL := cfg.OLHURL + "/api/gpuTemp/clean"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, apiURL, nil)
	if err != nil {
		return 0, err
	}
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return 0, fmt.Errorf("GPU temp API returned %s", resp.Status)
	}
	var r cpuTempResponse
	if err := json.NewDecoder(resp.Body).Decode(&r); err != nil {
		return 0, err
...
func fetchMetrics(ctx context.Context, cfg *ClientConfig) ([]Metric, error) {
	...
	// Fetch GPU temperature from separate API
	gpuTemp, err := fetchGpuTemp(ctx, cfg)
	if err != nil {
		logrus.Warnf("failed to fetch GPU temp: %v", err)
		gpuTemp = 0
	}
	...
	if gpuTemp != 0 {
		metrics = append(metrics, Metric{
			Measurement: "openlinkhub",
			Tags:        map[string]string{"host": cfg.OLHHost},
			Fields:      map[string]interface{}{"gpu_temp_c": gpuTemp},
			Timestamp:   now,
		})
	}
	return metrics, nil
